...
int decode(int size, const char* str, int* out_size, char** output) {
  *output = (char*) malloc((size_t)decode_size(size) );
  char *out = *output;
...
void encode(int size, const char* str, int* out_size, char** output) {
  *output = (char*) malloc((size_t)encode_size(size) );
  char *out = *output;
  const char* ends = str + (size - size % 3);
  uint n;
  while (str != ends) {
    uint32_t n = 0; //__builtin_bswap32(*(uint32_t*)str);
    *out++ = chars[(n >> 26) & 63];
    *out++ = chars[(n >> 20) & 63];
    *out++ = chars[(n >> 14) & 63];
    *out++ = chars[(n >> 8) & 63];
...
int EAPP_ENTRY entry() {
  init_decode_table();

  const int STR_SIZE = 10000000;
  const int TRIES = 100;

  char *str = (char*) malloc((size_t)STR_SIZE + 1);
